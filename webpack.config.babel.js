/* eslint-disable import/no-dynamic-require, no-console, global-require, no-restricted-syntax */
...
function load(file) {
  ...
  const conf = (Array.isArray(wp) ? wp : [wp]).map(config =>
    new WebpackConfig().merge(config).toObject(),
  );

  // console.warn('FILE: ', wp);
...
// console.warn('CONFIGuration: ', toExport);
